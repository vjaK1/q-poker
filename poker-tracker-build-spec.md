...
- Your bankroll card: lifetime net + the bankroll chart (§4.8) and nothing else. Decluttered 2026-09-07: no "this month" line and no text row; touching the chart shows a small label inside the plot (date, that night's net, bankroll after) that disappears when the finger lifts.
...
- **Launch screen** (2026-09-15): the Q.Poker mark (`icon-512.png`) breathing on the splash ground (`--splash-bg`, the manifest's `#030209`) while the login check and the live state load. No text. Held at least 1.5s (a full breath; raised from 0.8s the same day) so a fast load never flashes it; `index.html` paints the same markup before React loads so nothing cream shows in between. Launch only: in-page loads stay as "Loading…" text.
