...
- Off the books (2026-09-15, spec §4.11): `sessions.off_books`. `countsForStats` in derive.ts (saved and not off books) is the only gate for the Board, series and personal stats. Off-books nights stay in the Sessions tab with an "Off books" tag; exports and settle-up are deliberately untouched. Set by the Start tickbox, flippable on the live screen and session detail; the quick-start roster follows the tickbox.
- Launch screen (2026-09-15, spec §6): `LaunchScreen` (the mark breathing on `--splash-bg`, no text, 0.8s minimum hold, same markup pre-painted in index.html) replaces only the two boot "Loading…" states; in-page loaders stay text.
